...
def normalize(x, axis=-1):
    ...
    """
    x = 1. * x / (torch.norm(x, 2, axis, keepdim=True).clamp(min=1e-12).expand_as(x))
    return x
...
class OIM(autograd.Function):
    ...
    def backward(self, grad_outputs):
        ...
        for x, y in zip(inputs, targets):
            ...
            #self.lut[y] /= self.lut[y].norm().clamp(min=1e-12)
            #self.lut[y]=torch.norm(self.lut[y], 2, 1, True).clamp(min=1e-12).expand_as(self.lut[y])
            # self.lut[y]=normalize(self.lut[y],-1)
