...
def main():
    ...
    parser.add_argument('--resume', '-r',default='sysu_id_epoch10_baseline_gloid_debug_pyramid_attention_drop_0.0_lr_1.0e-02_dim_512_resnet50_best.t',type=str, help='resume from checkpoint')
    ...
    parser.add_argument('--use_cbam', default=False, type=bool,
    ...
    def attention_map_show(net,img,feature,label,batch_idx):
        ...
        net._modules.get("thermal_net").register_forward_hook(hook_feature_ther)
        ...
        # vis_weight_softmax = np.squeeze(vis_params[-2].cpu().data.numpy())
        weight_softmax = np.matmul(parm['classifier.classifier.1.weight'],parm['feature.feat_block.0.weight'])
        #-7 -15
    ...
    def show_cam(img, CAMs,label,batch_idx):
        ...
        plt.imshow(cam)
        plt.axis('off')
        plt.savefig('./image/attention/baseline_RGB_'+str(label.numpy())+'_'+str(batch_idx)+'.eps')
        plt.savefig('./image/attention/baseline_RGB_'+str(label.numpy())+'_'+str(batch_idx)+'.png')
        plt.show()
